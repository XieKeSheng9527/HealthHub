<template>
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="计划目标" prop="jihuamubiao">
					<el-input  v-model="ruleForm.jihuamubiao" placeholder="计划目标" clearable  :readonly="ro.jihuamubiao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="计划目标" prop="jihuamubiao">
					<el-input v-model="ruleForm.jihuamubiao" placeholder="计划目标" readonly></el-input>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.jihuaneirong = this.ruleForm.jihuaneirong.replace(reg,'../../../springboot9a097266/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yundongjihua/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yundongjihuaCrossAddOrUpdateFlag = false;
						 this.parent.search();
