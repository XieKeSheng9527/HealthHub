<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('jiankangxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('jiankangxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('jiankangxinxi','体重统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						体重统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiankangxinxi','血压统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						血压统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiankangxinxi','心率统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						心率统计
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiankangxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiankangxinxi','评估')" @click="jiankangpingguCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								评估
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiankangxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jiankangxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="tizhongChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xueyaChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import jiankangpingguCrossAddOrUpdate from "../jiankangpinggu/add-or-update";
	export default {
		...
		methods: {
			...
			jiankangpingguCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var tizhongChart1 = echarts.init(document.getElementById("tizhongChart1"),'macarons');
        this.$http({
            url: "jiankangxinxi/sectionStat/tizhong/riqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    yAxis1.push(parseFloat((res[i].瘦)));
                    yAxis2.push(parseFloat((res[i].偏瘦)));
                    yAxis3.push(parseFloat((res[i].中等)));
                    yAxis4.push(parseFloat((res[i].偏胖)));
                    yAxis5.push(parseFloat((res[i].肥胖)));
                    pArray.push({
                        name: res[i].tizhong
                    })
                }
                ...
                // 使用刚指定的配置项和数据显示图表。
                tizhongChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tizhongChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var xueyaChart2 = echarts.init(document.getElementById("xueyaChart2"),'macarons');
        this.$http({
            url: "jiankangxinxi/sectionStat/xueya/riqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].riqi);
                    yAxis1.push(parseFloat((res[i].低)));
                    yAxis2.push(parseFloat((res[i].偏低)));
                    yAxis3.push(parseFloat((res[i].正常)));
                    yAxis4.push(parseFloat((res[i].偏高)));
                    yAxis5.push(parseFloat((res[i].高)));
                    pArray.push({
                        name: res[i].xueya
                    })
                }
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "jiankangxinxi/sectionStat/xinlv/riqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].riqi);
                    yAxis1.push(parseFloat((res[i].低)));
                    yAxis2.push(parseFloat((res[i].正常)));
                    yAxis3.push(parseFloat((res[i].高)));
                    pArray.push({
                        name: res[i].xinlv
                    })
                }
	...
	jiankangxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jiankangxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
