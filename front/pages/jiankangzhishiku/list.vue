		...
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 4rpx","borderRadius":"16rpx","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.zhishimingcheng" type="text" placeholder="Knowledge Name" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing01" :style='{"color":"#a5ccc4","left":"0px","textAlign":"center","display":"none","width":"80rpx","fontSize":"40rpx","lineHeight":"68rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.zhishileixing" type="text" placeholder="Knowledge Name" ></input>
				</view>
			...
			<view :style='{"margin":"132rpx 0px 0 0px","position":"absolute","right":"40rpx","flexWrap":"wrap","display":"flex"}'>
			...
			<view :style='{"width":"100%","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"padding":"20rpx 0","margin":"0","whiteSpace":"nowrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					<view :class='categoryName === item.zhishileixing ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.zhishileixing)">{{item.zhishileixing}}</view>
				</scroll-view>
			
			<!-- 样式2 -->
			<view class="list" :style='{"width":"100%","padding":"0","margin":"92rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.zhishitupian)" :src="product.zhishitupian.split(',')[0]"></image>
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.zhishitupian?baseUrl+product.zhishitupian.split(',')[0]:''"></image>

					<view class="listmr" :style='{"width":"calc(100% - 180rpx)","padding":"0","margin":"0","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.zhishimingcheng}}</view>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.zhishileixing}}</view>
						...
						<view :style='{"padding":"0 0px","margin":"0 20rpx 0 0"}'>
							...
							<text :style='{"color":"inherit","lineHeight":"1.5","fontSize":"24rpx"}'>{{product.$pubPeople}}</text>
						</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhishiku','修改')) || (!userid && isAuthFront('jiankangzhishiku','修改'))" @click.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhishiku','删除')) || (!userid && isAuthFront('jiankangzhishiku','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('zhishileixing', {page:1,limit:100});
            } else {
                res = await this.$api.list('zhishileixing', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,zhishileixing:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiankangzhishiku`, params);
                } else {
                    res = await this.$api.list(`jiankangzhishiku`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiankangzhishiku', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiankangzhishiku`, searchForm);
                } else {
                    res = await this.$api.list(`jiankangzhishiku`, searchForm);
                }
				// 如果是第一页数据置空
