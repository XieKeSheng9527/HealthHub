<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
			...
			<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
				...
				<view class="listmr">
					...
					<view class="col3 f30 elip mb15">
						<button v-if="item.isdone=='开放'" class="cu-btn bg-green sm">open</button>
						<button v-if="item.isdone=='关闭'" class="cu-btn bg-red sm">private</button>
					</view>
					<!-- <view class="colb f24 lh35">{{item.content.length>50?item.content.substring(0,50)+'... ...':item.content}}</view> -->
					<view v-if="item.addtime" class="colb f24 lh35">Release time{{item.addtime}}</view>
					<view class="flex flex-align-center mt20">
						<view class="colb f25">
							<button @tap="onDetailTap(item.id)" class="cu-btn bg-green shadow">view</button>
		...
		<!--list end-->
		<button class="add-btn" @click="onUpdateTap()">new</button>
	</mescroll-uni>
</template>
